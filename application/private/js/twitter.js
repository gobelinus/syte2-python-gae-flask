...
angular.module('clientApp')
  ...
  .controller('TwitterCtrl', ['$scope', '$rootScope', '$http', '$window',
    function($scope, $rootScope, $http, $window) {
      ...
      $scope.openPicture = function(picture) {
        if (picture.tweetID || picture.tweet_id) {
          $window.open('https://twitter.com/' + $scope.user.username + '/status/' + (picture.tweetID || picture.tweet_id));
        }
      };

      $scope.openPost = function(item, index) {
        //console.log('openPost with index = ' + index + ' and item = ');
        //console.dir(item);
        $scope.openTwitterPost(item, index);
      };
    }
  ])
  ...
  .directive('twitterPost', function() {
    return {
      ...
      templateUrl: '/partials/twitter/post.html',
      link: function(scope) {
        scope.openPost = function(item, index) {
          scope.$root.openTwitterPost(item, index);
        };
      }
    };
  })
